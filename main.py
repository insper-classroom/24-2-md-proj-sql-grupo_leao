...
mysql_url = f"mysql+mysqlconnector://{user}:{secret_key}@{server}"
mysql_url_db = f"mysql+mysqlconnector://{user}:{secret_key}@{server}/{database_name}"
...
# CRUD for Locals
@app.post("/locals/", response_model=Local, tags=["Locals"], summary='Creates a new local', description="Creates a new local (location) by accepting details such as name, address, city, capacity, and phone number. The local information is stored in a database.")
...
@app.patch("/locals/{local_id}", response_model=Local, tags=["Locals"], summary='Updates a specific local', description="Partially updates an existing local by providing the local ID and the new details to be updated. Only the fields that are passed will be updated in the database.")
...
@app.delete("/locals/{local_id}", tags=["Locals"], summary='Deletes a local', description="Deletes a specific local from the database by providing the local ID. If the local is found, it will be removed from the database, otherwise an error will be returned.")
...
@app.post("/eventtypes/", response_model=EventType, tags=["Event Type"], summary='Creates a new event type', description="Creates a new event type by accepting details such as category, description, and target audience. The event type information is stored in a database.")
...
@app.patch("/eventtypes/{event_type_id}", response_model=EventType, tags=["Event Type"], summary='Updates a specific event type', description="Partially updates an existing event type by providing the event type ID and the new details to be updated. Only the fields that are passed will be updated in the database.")
...
@app.delete("/eventtypes/{tipo_evento_id}", tags=["Event Type"], summary='Deletes an event type', description="Deletes a specific event type from the database by providing the event type ID. If the event type is found, it will be removed from the database, otherwise an error will be returned.")
...
# CRUD for Events
@app.post("/events/", response_model=Event, tags=["Event"], summary='Creates a new event', description="Creates a new event by accepting details such as name, description, start and end dates, times, location, and event type. The event information is stored in a database.")
...
@app.patch("/events/{event_id}", response_model=Event, tags=["Event"], summary='Updates a specific event', description="Partially updates an existing event by providing the event ID and the new details to be updated. Only the fields that are passed will be updated in the database.")
...
@app.delete("/eventos/{event_id}", tags=["Event"], summary='Deletes an event', description="Deletes a specific event from the database by providing the event ID. If the event is found, it will be removed from the database, otherwise an error will be returned.")
...
#CRUD for event_local_links
@app.post("/local_event/", response_model=LocalEventLink, tags=["LocalEvent"],
          summary="Creates a new local-event relationship",
          description="Creates a new relationship between a location and an event by accepting the local_id and event_id. The relationship is stored in a database.")
...
@app.patch("/local_event/{event_type_id}", response_model=LocalEventLink, tags=["LocalEvent"], 
         summary='Updates a specific local event link', 
         description="Partially updates an existing event type by providing the event type ID and the new details to be updated. Only the fields that are passed will be updated in the database.")
def update_local_event(local_event_id: int, local_event: LocalEventLinkUpdate, session: SessionDep):
    local_event_db = session.get(LocalEventLink, local_event_id)
    if not local_event_db:
        raise HTTPException(status_code=404, detail="Local Event Link not found")
    local_event_data = local_event.model_dump(exclude_unset=True)
    local_event_db.sqlmodel_update(local_event_data)
    session.add(local_event_db)
    session.commit()
    session.refresh(local_event_db)
    return local_event_db

@app.delete("/local_event/{link_id}", tags=["LocalEvent"])
